...
export const sockName = (sock: string): string => baseName(sock);
...
function matchSessions(
  children: readonly DeepReadonly<Proc>[],
  listed: readonly DeepReadonly<Info>[],
): Map<number, number> {
...
function relays(h: DeepReadonly<Host>, p: DeepReadonly<Proc>): boolean {
  return h.relays.some((r) => (r.id !== null && r.id === p.session) || (r.pid !== null && r.pid === p.pid));
}

export function orphanRows(r: DeepReadonly<Orphans>, listed: readonly DeepReadonly<Info>[]): OrphanRow[] {
