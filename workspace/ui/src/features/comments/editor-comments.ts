...
import { keepVisible } from '#editor/context-view';
...
/** The comments feature registers its sync here, for the same reason as `onCursor`: after any change
 *  to a document that carries marks, it copies their new lines back into S. */
...
export const commentsExtension: Extension = [
  ...
  keepVisible.of((state) => marksOf(state)
    .map((m) => ({ first: state.doc.lineAt(m.from).number, last: state.doc.lineAt(m.to).number }))),
];
