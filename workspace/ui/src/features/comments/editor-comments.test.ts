...
import { buildState, rejectChunk, replaceDoc, type ViewKind } from '#editor/editor';
import {
  commentHost, commentsExtension, commentSpan, lineRange, marksOf, onComments, PosMapper, setMarks, type Edit,
} from './editor-comments';

async function mount(kind: ViewKind, doc: string, original: string | null = null) {
  const parent = document.createElement('div');
  document.body.appendChild(parent);
  const state = await buildState(kind, 'src/x.ts', doc, original, () => {}, [commentsExtension]);
...
describe('marks', () => {
  ...
  it('give each mark a host for React to portal into, and tell the comments feature about edits', async () => {
    const run = vi.fn();
    onComments.run = run;
    const view = await mount('plain', 'a\nb\n');
