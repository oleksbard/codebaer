...
import { foldToChanges, keepRanges, keepVisible } from './context-view';
...
describe('lines a feature keeps visible', () => {
  it('stay out of the folds around the changes', async () => {
    const orig = Array.from({ length: 20 }, (_, i) => `l${i + 1}`).join('\n') + '\n';
    const doc = orig.replace('l15\n', 'changed\n');
    const keep = keepVisible.of(() => [{ first: 2, last: 2 }]);
    const s = await buildState('unstaged', 'x.txt', doc, orig, () => {}, [keep]);
    expect(keepRanges(s)).toEqual([{ first: 2, last: 2 }, { first: 12, last: 18 }]);
  });
});
