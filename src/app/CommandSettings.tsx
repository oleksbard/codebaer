...
type Editing = CustomCommand | 'new' | null;

/** Entries have no id of their own, and an index key would hand one row's form to its neighbour on a delete. */
const keys = new WeakMap<CustomCommand, number>();
let nextKey = 0;
function keyOf(c: CustomCommand): number {
  let k = keys.get(c);
  if (k === undefined) {
    k = nextKey++;
    keys.set(c, k);
...
function CommandRow({ c, onEdit, onDelete }: { c: CustomCommand; onEdit(): void; onDelete(): void }) {
