...
Naming: the product is "CodeBär", and the repo, crate, binary and bundle id use `codebaer`. The folder name `reviewbaer` is older than the rename.
...
CI (`.github/workflows/ci.yml`) runs all of these, plus `pnpm tauri build --ci`, on macOS. Run the ones that cover your change before you call it done.
